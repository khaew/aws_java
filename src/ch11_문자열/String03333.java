...
public class String03333 {
	...
	public static void main(String[] args) {
		...
		String[] addressArray3 = new String[0];
			...
			String[] temp = new String[addressArray2.length +1];
				//기존의 배열에 있는 정보를 새로운 배열에 올긴다.
			for( int j  = 0; j<addressArray2.length; j++) {
				temp[j] = addressArray2[j];
			}
				//마지막 배열 공간에 새로운 값을 대입한다.
			temp[addressArray2.length] = addressArray[i];
				//새로운 배열을 기존의 배열 변수에 대입한다.
			addressArray2 = temp;
			}else if(addressArray[i].contains("창원시")) {
				//기존 배열의 공간보다 1이 큰 새로운 배열을 생성한다.
				String[] temp = new String[addressArray3.length +1];
					//기존의 배열에 있는 정보를 새로운 배열에 올긴다.
				for( int j  = 0; j<addressArray3.length; j++) {
					temp[j] = addressArray3[j];
				}
					//마지막 배열 공간에 새로운 값을 대입한다.
				temp[addressArray3.length] = addressArray[i];
					//새로운 배열을 기존의 배열 변수에 대입한다.
				addressArray3 = temp;
					}
		}
